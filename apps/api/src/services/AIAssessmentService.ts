...
export class AIAssessmentService {
  ...
  static async processJob(job: any) {
    const { materialId, competencyId, difficulty, count } = job.metadata;
    ...
    const result = await aiService.generateMCQs(job.requesterId, job._id.toString(), textChunks, competencyName, difficulty, count);
    ...
    for (const q of result.questions) {
      ...
      if (uniqueOptions.size !== optionTexts.length) continue;
