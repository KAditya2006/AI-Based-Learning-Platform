...
import { IntegrationConfig, IntegrationSyncJob, SyncJobStatus, IntegrationStatus, LearningResource, ResourceSource, Competency } from '../models';
...
export class IntegrationSyncService {
  ...
  static async syncCatalog(providerId: string, correlationId: string): Promise<string> {
    ...
    // Enqueue the actual background work with a valid dummy ObjectId to prevent CastErrors in JobService
    const backgroundTaskId = new mongoose.Types.ObjectId().toString();
    JobService.executeAsync(backgroundTaskId, async () => {
      try {
        ...
        // Ensure all tags map to Competencies
        ...
        // Find default framework for auto-generated comps
        ...
        // Process and upsert via bulkWrite
        const bulkOps = [];
        for (const er of externalResources) {
          ...
          bulkOps.push({
            updateOne: {
              filter: { provider: er.provider, externalUrl: er.externalId },
              update: {
                $set: {
                  title: er.title,
